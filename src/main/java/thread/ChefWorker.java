...
//                    Thread.sleep(100);  // 일감 없으면 잠깐 대기
	 ...
	 * 
	 * <p>
	 * 여러 요리사 스레드가 동시에 호출할 수 있으므로 synchronized로 동기화하여 같은 주문을 중복으로 가져가는 것을 방지한다.
	 * </p>
	 * 
	...
	private Order findWork() {
		synchronized (queueManager) {
			...
			OrderQueue targetQueue = null;

			// 1. 큐 포화도 기반 긴급 작업 탐색 (우선순위 1)
			for (MenuItem menu : MenuItem.values()) {
				OrderQueue queue = queueManager.getMenuQueue(menu);
				// 큐의 현재 크기가 임계치(예: 8개 이상)를 넘었는지 확인
				// OrderQueue 내부에 size()와 capacity()가 정의되어 있어야 함
				if (queue.size() >= 8) { // 임계값은 설정에 따라 조정
					targetOrder = queue.poll();
					if (targetOrder != null) {
			...
			// 2. 주문 번호 기반 일반 작업 탐색 (우선순위 2 - 기존 로직)
			...
			if (earliestQueue != null) {
				earliestQueue.poll();
				this.currentOrder = earliestOrder;
				this.currentMenu = earliestMenu;
				logger.info(
						"[요리사#" + id + "] 주문#" + earliestOrder.getOrderId() + " " + earliestMenu.getName() + " 조리 시작");
